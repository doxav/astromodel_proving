        ...
        rows.append(
            {
                ...
                "source_path": str(Path(record.source_path).name),
                "source_relpath": str(Path("data") / "2_K+ Pumps Data" / Path(record.source_path).name),
...
def build_trace_source_summary(initial_fit_dir: str | Path) -> pd.DataFrame:
    ...
    names = {row["trace_source"] for row in rows}
    if "CONTROL_TRACES_old.csv" not in names:
        rows.append(
            {
                "trace_source": "CONTROL_TRACES_old.csv",
                "condition": "CONTROL",
                "rows": np.nan,
                "n_columns": np.nan,
                ...
                "uses_header": np.nan,
                "file_size_bytes": 0,
                "dataset_kind": "removed_legacy_optuna_trace_csv",
                "exists": False,
                "provenance_status": "removed_not_used",
            }
        )
    for row in rows:
        row.setdefault("exists", row.get("file_size_bytes", 0) > 0)
        row.setdefault("provenance_status", "present")
...
def run_step00_provenance(
    project_root: str | Path,
    relative_tolerance: float = 1e-2,
    output_dir: str | Path | None = None,
    recompute_objectives: bool = True,
                ...
                [
                    {
                        **row,
                        ...
                        "chosen_status": "not_recomputed",
                    }
                    for row in db_summary.to_dict("records")
                ]
            )
    else:
