...
def parse_atf(path: str | Path) -> dict[str, Any]:
    path = Path(path)
    if not path.exists() and not path.stem.upper().endswith("_CONTROL"):
        control_alias = path.with_name(f"{path.stem}_CONTROL{path.suffix}")
        if control_alias.exists():
            path = control_alias
...
# Step 04 compatibility helpers -------------------------------------------------
# Step 02 keeps the full feature-extraction implementation above.  Step 04 needs
# a compact in-memory trace API for simulated sweeps; expose it here so the same
# feature names and redundancy policy are reused instead of maintaining an orphan
# second extractor.
...
def extract_features_from_trace(
    time_s: np.ndarray,
    vm_mV: np.ndarray,
    current_nA: np.ndarray | None = None,
    onset_s: float | None = None,
    offset_s: float | None = None,
) -> dict[str, float | bool]:
    """Extract the Step 04 scoring features from a single voltage trace."""
    t = np.asarray(time_s, dtype=float)
    v = np.asarray(vm_mV, dtype=float)
    if t.shape != v.shape or len(t) < 3:
        raise ValueError("time_s and vm_mV must be same-length arrays with at least three samples")
    finite = np.isfinite(t) & np.isfinite(v)
    if finite.sum() < 3:
        return {feature: np.nan for feature in FEATURE_COLUMNS} | {"plateau_reached": False, "has_undershoot": False}
    t = t[finite]
    v = v[finite]
    if current_nA is not None:
        current = np.asarray(current_nA, dtype=float)[finite]
        active = np.abs(current) > max(1e-9, 0.05 * float(np.nanmax(np.abs(current))))
        if active.any():
            onset = float(t[np.argmax(active)])
            offset = float(t[len(active) - np.argmax(active[::-1]) - 1])
        else:
            onset = float(t[0] + 0.25 * (t[-1] - t[0]))
            offset = float(t[0] + 0.75 * (t[-1] - t[0]))
    else:
        onset = float(t[0] + 0.25 * (t[-1] - t[0]))
        offset = float(t[0] + 0.75 * (t[-1] - t[0]))
    if onset_s is not None:
    ...
    baseline_mask = t < onset
    if not baseline_mask.any():
        baseline_mask = np.arange(len(t)) < max(1, len(t) // 10)
    baseline = float(np.nanmedian(v[baseline_mask]))
    centered = v - baseline
    stim_mask = (t >= onset) & (t <= offset)
    post_mask = t > offset
    peak = float(np.nanmax(centered[stim_mask])) if stim_mask.any() else float(np.nanmax(centered))
    stim_end_mask = (t >= max(onset, offset - 1.0)) & (t <= offset)
    stim_end = float(np.nanmedian(centered[stim_end_mask])) if stim_end_mask.any() else peak

    def _slope(mask: np.ndarray) -> float:
        if int(mask.sum()) < 2:
            return np.nan
        return float(np.polyfit(t[mask], centered[mask], 1)[0])

    rise_mask = (t >= onset) & (t <= min(offset, onset + max(1.0, 0.25 * (offset - onset))))
    plateau_mask = (t >= max(onset, offset - max(1.0, 0.25 * (offset - onset)))) & (t <= offset)
    decay_mask = (t >= offset) & (t <= min(t[-1], offset + max(1.0, 0.25 * (t[-1] - offset))))
    return_mask = t >= max(offset, t[-1] - max(1.0, 0.2 * (t[-1] - t[0])))
    post_min = float(np.nanmin(centered[post_mask])) if post_mask.any() else np.nan
    undershoot = max(0.0, -post_min) if np.isfinite(post_min) else 0.0
    ...
    return {
        "baseline_mV": baseline,
        "stim_onset_s": onset,
        "stim_offset_s": offset,
        "peak_depolarization_mV": peak,
        "stim_end_depolarization_mV": stim_end,
        "rise_slope_mV_per_s": rise_slope,
        "rise_tau_s": rise_tau,
        "plateau_slope_mV_per_s": plateau_slope,
        "plateau_reached": bool(np.isfinite(plateau_slope) and abs(plateau_slope) < max(0.1, abs(peak) * 0.05)),
        "undershoot_magnitude_mV": float(undershoot),
        "has_undershoot": bool(undershoot > 0),
        "decay_slope_mV_per_s": decay_slope,
        "decay_tau_s": decay_tau,
        "return_slope_mV_per_s": return_slope,
    }
...
def build_feature_table(cells: Any) -> Any:
    """Build Step 04 feature tables from loaded cells or an ATF directory.

    A sequence input returns the feature DataFrame used by six-sweep fitting.
    A path input returns ``(feature_df, cell_dict)`` for the multisweep driver.
    """
    from .atf_io import load_all_cells

    if isinstance(cells, (str, Path)):
        loaded = load_all_cells(Path(cells))
        feature_df = build_feature_table(loaded)
        cell_dict: dict[str, dict[str, Any]] = {}
        for cell in loaded:
            sweeps: dict[int, dict[str, Any]] = {}
            for sweep in cell.sweeps:
                features = extract_features_from_trace(sweep.time_s, sweep.vm_mV, sweep.current_nA)
                sweeps[int(sweep.sweep)] = {
                    "time_s": sweep.time_s,
                    "vm_mV": sweep.vm_mV,
                    "voltage_mV": sweep.vm_mV,
                    "current_nA": sweep.current_nA,
                    "current_na": int(sweep.current_na),
                    "features": features,
                }
            cell_dict[cell.file_id] = {
                "meta": {
                    "file_id": cell.file_id,
                    "file": cell.file_name,
                    "region": cell.region,
                    "condition": cell.condition,
                    "group_label": f"{cell.region}_{cell.condition}",
                    "cell_label": cell.file_id,
                },
                "sweeps": sweeps,
            }
        return feature_df, cell_dict

    rows: list[dict[str, Any]] = []
    for cell in cells:
        if isinstance(cell, dict):
            meta = dict(cell["meta"])
            iterable = cell["sweeps"].items()
            for sweep, sweep_data in sorted(iterable):
                if "features" in sweep_data:
                    features = dict(sweep_data["features"])
                else:
                    features = extract_features_from_trace(sweep_data["time_s"], sweep_data.get("vm_mV", sweep_data.get("voltage_mV")), sweep_data.get("current_nA"))
                row = {**meta, "sweep": int(sweep), "current_na": int(sweep_data.get("current_na", 0))}
                row.update(features)
                rows.append(row)
        else:
            for sweep in cell.sweeps:
                features = extract_features_from_trace(sweep.time_s, sweep.vm_mV, sweep.current_nA)
                row = {"file_id": cell.file_id, "file": cell.file_name, "region": cell.region, "condition": cell.condition, "sweep": int(sweep.sweep), "current_na": int(sweep.current_na)}
                row.update(features)
                rows.append(row)
    return pd.DataFrame(rows)
