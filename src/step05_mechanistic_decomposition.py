...
from .astro_model import CURRENT_DICT_K_BATH_VALUES, VALID_CURRENTS, simulate_with_hidden_outputs
...
def load_step04_accepted_ensemble(project_root: Path | str, source_path: Path | str | None = None, max_candidates: int | None = None) -> tuple[pd.DataFrame, Path]:
    ...
    candidates = [
        ...
        root / "outputs" / "step04_cell_specific_multisweep" / "accepted_candidates.csv",
    ]
    path = Path(source_path).resolve() if source_path is not None else next((p for p in candidates if p.exists()), candidates[0])
    if not path.exists():
    ...
    missing = sorted(set(IDENTITY_COLUMNS + EFFECTIVE_COLUMNS + ["g_kir", "gl_a", "zth", "zs", "eps"]) - set(df.columns))
    if missing:
        raise ValueError(f"Step 04 ensemble is missing required columns: {missing}")
    df = df[df["accepted"]].copy()
    df["step04_source_path"] = str(path.relative_to(root) if path.is_relative_to(root) else path)
...
def reconstruct_flat_params(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    current_gain = float(row.get("k_bath_gain", 1.0))
    condition = str(row.get("condition", "CONTROL")).upper()
    if condition == "MFA_BA":
        protocol_condition = "BARIUM"
    else:
        protocol_condition = condition
    middle_default = CURRENT_DICT_K_BATH_VALUES["100"][1]
    params = {
        "d": 1.0,
        ...
        "K_bath_value_middle": middle_default * current_gain,
        "_protocol_condition_hint": protocol_condition,
    }
    return params
...
def simulate_candidate_sweeps(candidate: Mapping[str, Any], config: Step05Config) -> pd.DataFrame:
    """Simulate all six sweeps for one accepted candidate and return flux rows."""

    params = reconstruct_flat_params(candidate)
    ...
    for sweep, current_na in enumerate(VALID_CURRENTS, start=1):
        ...
        base.update({"sweep": sweep, "current_na": int(current_na)})
        try:
            sim = simulate_with_hidden_outputs(
                params,
                {"experiment_type": condition, "current_na": int(current_na), "t_eval_ms": time_ms},
            )
...
def build_candidate_mechanism_table(fit_mechanisms: pd.DataFrame) -> pd.DataFrame:
    ...
    if ok.empty:
        raise ValueError("No successful Step 05 hidden-output simulations were available for clustering.")
    ...
    first_cols = [c for c in EFFECTIVE_COLUMNS + ["g_kir", "gki", "gl_a", "zth", "zs", "eps", "k_bath_gain", "switching_function"] if c in ok.columns]
    ...
    out["log10_gap_to_kir_integral_ratio_mean"] = np.log10(np.clip(out["gap_to_kir_integral_ratio_mean"].astype(float), 1e-18, None))
    out["dominant_mechanism"] = np.select(
        [out["gap_fraction_mean"] >= 0.6, out["kir_fraction_mean"] >= 0.6, out["leak_fraction_mean"] >= 0.6],
        ["Gap", "Kir", "Leak"],
        default="Mixed",
    )
    return out
...
def _standardize(df: pd.DataFrame, columns: list[str]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def assign_mechanism_clusters(candidate_table: pd.DataFrame, config: Step05Config) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    df = candidate_table.copy()
    x, mean, scale = _standardize(df, MECHANISM_FEATURES)
    k = min(max(1, int(config.n_clusters)), len(df))
    if k == 1:
...
def bootstrap_cluster_stability(clustered: pd.DataFrame, centers: np.ndarray, mean: np.ndarray, scale: np.ndarray, config: Step05Config) -> pd.DataFrame:
    rng = np.random.default_rng(int(config.random_seed))
    cells = clustered["file_id"].drop_duplicates().to_numpy()
    original_labels = clustered["mechanism_cluster"].to_numpy()
    rows: list[dict[str, Any]] = []
    if len(cells) < 2 or len(clustered) < 3 or config.bootstrap_iterations <= 0:
        return pd.DataFrame([{"bootstrap_iteration": 0, "n_cells": len(cells), "n_candidates": len(clustered), "coassignment_stability": np.nan, "stability_status": "insufficient_evidence"}])
    for i in range(int(config.bootstrap_iterations)):
        sampled = rng.choice(cells, size=len(cells), replace=True)
        boot = pd.concat([clustered[clustered["file_id"] == cell] for cell in sampled], ignore_index=True)
        ...
        orig = boot["mechanism_cluster"].str.replace("M", "", regex=False).astype(int).to_numpy() - 1
        rows.append(
            {
                "bootstrap_iteration": i + 1,
                "n_cells": int(len(np.unique(sampled))),
                "n_candidates": int(len(boot)),
                "coassignment_stability": _pairwise_coassignment_score(orig, boot_labels),
                "stability_status": "stable" if _pairwise_coassignment_score(orig, boot_labels) >= 0.75 else "unstable",
            }
        )
    return pd.DataFrame(rows)


def summarize_region_enrichment(clustered: pd.DataFrame, small_stratum_min_cells: int = 3) -> pd.DataFrame:
    counts = clustered.groupby(["region", "condition", "mechanism_cluster"], as_index=False).agg(
        n_candidates=("candidate_id", "nunique"),
        n_cells=("file_id", "nunique"),
    )
    totals = counts.groupby(["region", "condition"], as_index=False).agg(total_candidates=("n_candidates", "sum"), total_cells=("n_cells", "sum"))
    out = counts.merge(totals, on=["region", "condition"], how="left")
    out["candidate_fraction"] = out["n_candidates"] / out["total_candidates"].replace(0, np.nan)
    out["small_stratum_flag"] = out["total_cells"] < int(small_stratum_min_cells)
    out["interpretation_scope"] = "population_level_cell_association_not_phenotype_claim"
    return out


def select_representatives(clustered: pd.DataFrame, config: Step05Config) -> pd.DataFrame:
    df = clustered[clustered["accepted"].astype(bool)].copy()
    if df.empty:
        return pd.DataFrame(columns=list(clustered.columns) + ["representative_rank", "selection_reason", "claim_scope"])
    x, _, _ = _standardize(df, MECHANISM_FEATURES)
    selected = [int(np.argmin(df.get("K_o_recovery_error_mean", pd.Series(np.zeros(len(df)))).to_numpy(dtype=float)))]
...
def classify_geometry(clustered: pd.DataFrame, config: Step05Config) -> pd.DataFrame:
    clusters = sorted(clustered["mechanism_cluster"].drop_unique() if hasattr(clustered["mechanism_cluster"], "drop_unique") else clustered["mechanism_cluster"].unique())
    if len(clusters) < 2:
        return pd.DataFrame([{"cluster_a": clusters[0] if clusters else "none", "cluster_b": "none", "n_interpolation_points": 0, "interpolation_status": "not_run", "geometry_classification": "insufficient_evidence", "validation_status": "pending_step06"}])
    rows: list[dict[str, Any]] = []
    centers = clustered.groupby("mechanism_cluster")[EFFECTIVE_COLUMNS + ["gap_fraction_mean", "kir_fraction_mean", "leak_fraction_mean"]].median(numeric_only=True)
    functional_low = float(clustered.get("K_o_recovery_error_mean", pd.Series([0.0])).quantile(0.05))
    functional_high = float(clustered.get("K_o_recovery_error_mean", pd.Series([0.0])).quantile(0.95))
    for a, b in combinations(clusters, 2):
        ca = centers.loc[a]
        cb = centers.loc[b]
        accepted_like = True
        for alpha in np.linspace(0.0, 1.0, int(config.interpolation_points)):
            interp = (1 - alpha) * np.log10(np.clip(ca[EFFECTIVE_COLUMNS].astype(float), 1e-18, None)) + alpha * np.log10(np.clip(cb[EFFECTIVE_COLUMNS].astype(float), 1e-18, None))
            # Conservative functional proxy: interpolate observed recovery errors between cluster medians.
            err_a = float(clustered.loc[clustered["mechanism_cluster"] == a, "K_o_recovery_error_mean"].median())
            err_b = float(clustered.loc[clustered["mechanism_cluster"] == b, "K_o_recovery_error_mean"].median())
            err = (1 - alpha) * err_a + alpha * err_b
            accepted_like &= bool(functional_low - 1e-12 <= err <= functional_high + 1e-12)
        rows.append(
            {
                "cluster_a": a,
                "cluster_b": b,
                ...
                "geometry_classification": "compensation_manifold" if accepted_like else "separated_modes_pending_validation",
...
def build_claim_scope_table(clustered: pd.DataFrame, geometry: pd.DataFrame) -> pd.DataFrame:
    has_separated = geometry["geometry_classification"].astype(str).str.contains("separated_modes").any()
    headline = "candidate_mechanism_regimes_pending_validation" if has_separated else "accepted_compensation_or_mixed_mechanism_structure_pending_validation"
    return pd.DataFrame(
        [
            {
                ...
                "n_clusters": int(clustered["mechanism_cluster"].nunique()) if not clustered.empty else 0,
            },
            {
...
def compare_simulation_grid_performance(project_root: Path | str, max_candidates: int = 1) -> pd.DataFrame:
    rows = []
    for label, points in [("coarse", 90), ("default", 180)]:
        cfg = Step05Config(max_candidates=max_candidates, time_points=points, bootstrap_iterations=0, write_outputs=False)
        ...
        rows.append(
            {
                ...
                "n_clusters": result["mechanism_clusters"]["mechanism_cluster"].nunique(),
                "recommendation": "use_default_for_reviewer_outputs" if label == "default" else "use_coarse_for_debug_only",
            }
        )
    return pd.DataFrame(rows)


def run_step05_mechanistic_decomposition(project_root: Path | str, config: Step05Config | None = None, output_dir: Path | str | None = None) -> dict[str, pd.DataFrame | dict[str, Any]]:
    ...
    ensemble, source_path = load_step04_accepted_ensemble(root, max_candidates=config.max_candidates)
    sweep_tables = [simulate_candidate_sweeps(row.to_dict(), config) for _, row in ensemble.iterrows()]
    ...
    mean_stability = float(stability["coassignment_stability"].mean(skipna=True)) if "coassignment_stability" in stability else np.nan
    clustered["bootstrap_mean_coassignment_stability"] = mean_stability
    clustered["cluster_claim_scope"] = np.where(
        np.isfinite(mean_stability) & (mean_stability >= 0.75),
        "candidate_mechanism_regime_pending_step06_validation",
        "insufficient_stability_for_separated_mode_claim",
    )
    ...
    perf = compare_simulation_grid_performance(root, max_candidates=min(1, len(ensemble))) if config.write_outputs else pd.DataFrame()

    summary = {
        "step_name": "Step 05 — Mechanistic decomposition of accepted cell ensembles",
        "input_source": str(source_path.relative_to(root) if source_path.is_relative_to(root) else source_path),
        "config": asdict(config),
        "n_input_candidates": int(len(ensemble)),
        "n_candidate_sweep_rows": int(len(fit_mechanisms)),
        "n_successful_sweep_simulations": int((fit_mechanisms["simulation_status"] == "ok").sum()),
