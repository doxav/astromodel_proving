...
from src.astro_model import build_paramdict
...
def test_step05_loads_step04_ensemble_with_region_condition_contract(project_root):
    ...
    assert {"file_id", "region", "condition", "candidate_id", "P_gap_eff", "gamma_t_eff", "gamma_s_eff", "volume_ratio_wa_wo"}.issubset(ensemble.columns)
...
def test_effective_parameter_reconstruction_preserves_model_coordinates(project_root):
    ...
    assert np.isclose(astro["gama_t"] * astro["Sig_a"] / (astro["w_a"] * astro["F"]), float(row["gamma_t_eff"]))
    assert np.isclose(astro["gama_s"] * astro["Sig_a"] / (astro["w_a"] * astro["F"]), float(row["gamma_s_eff"]))
...
def test_step05_flux_rows_have_hidden_current_and_proxy_metrics(project_root):
    ...
    required = {"I_Kir_integral", "I_kgap_integral", "I_leak_integral", "K_o_peak", "gap_to_kir_integral_ratio", "proxy_validity_class"}
