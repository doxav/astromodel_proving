...
def test_step05_writes_required_outputs_and_conservative_claims(project_root):
    ...
    result = run_step05_mechanistic_decomposition(
        project_root,
        Step05Config(max_candidates=2, time_points=60, bootstrap_iterations=2, write_outputs=True),
        output_dir=out_dir,
    )
    required_files = {
    ...
    assert {"region", "condition", "mechanism_cluster", "cluster_claim_scope"}.issubset(clusters.columns)
...
def test_step05_representatives_preserve_step04_acceptance(project_root):
    result = run_step05_mechanistic_decomposition(
        project_root,
        Step05Config(max_candidates=3, time_points=60, bootstrap_iterations=0, write_outputs=False),
    )
    reps = result["representatives"]
    assert not reps.empty
    assert reps["accepted"].astype(bool).all()
    assert reps["claim_scope"].eq("candidate_mechanism_regime_pending_step06_validation").all()
