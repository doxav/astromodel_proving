...
def test_step05_enrichment_and_geometry_are_coherent(project_root):
    ...
    enrichment_clusters = set(
        result["region_mechanism_enrichment"]["mechanism_cluster"]
    )
    assert enrichment_clusters.issubset(clusters)
    assert (
        result["geometry_classification"]["validation_status"]
        .eq("pending_step06")
        .all()
    )


def test_step05_geometry_is_downgraded_when_cluster_evidence_is_insufficient(
    project_root,
):
    result = run_step05_mechanistic_decomposition(
        project_root,
    ...
    assert (
        result["geometry_classification"]["geometry_classification"]
        .eq("insufficient_evidence")
        .all()
    )
    assert (
        result["geometry_classification"]["geometry_screen_type"]
        .eq("not_run_insufficient_cluster_evidence")
        .all()
    )
...
def test_step05_notebook_executes_and_saves_auditable_copy(project_root):
    executed = execute_notebook(
        project_root / "analysis" / "05_mechanistic_decomposition.ipynb", project_root
    )
